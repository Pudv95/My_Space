...
long long countNonAttackingPlacements(int n) {
    long long totalPositions = 1LL * n * n * (n * n - 1) / 2;
    long long attackingPositions = 4LL * (n - 1) * (n - 2);
    return totalPositions - attackingPositions;
}
...
void solve(){
    ...
    cin>>n;
    for(int i=1;i<=n;i++){
        cout<<countNonAttackingPlacements(i)<<endl;
    }
}
